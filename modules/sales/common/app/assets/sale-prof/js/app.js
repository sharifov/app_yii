		...
		.factory('API', ['$http', 'Upload', function($http, Upload) {
			return {
				...
				loadProducts:     function(type_id, category_id) {
					return $http.get(apiUrl(settings.api.products), {params: {typeId: type_id, categoryId: category_id, promotionId:4}});
				},
				loadCategories:   function(type_id) {
					return $http.get(apiUrl(settings.api.categories), {params: {typeId:type_id, promotionId:4}});
				},
				loadTypes:        function() {
					return $http.get(apiUrl(settings.api.types), {params: {promotionId:4}});
				},
