...
/**
 * Class DealerPaymentPartner
 */
class DealerPaymentPartner extends BaseObject implements TransactionPartnerInterface
{
    public $id;
    ...
    public function getTitleForTransaction()
    {
        if (!array_key_exists($this->id, self::$_titles)) {
            self::$_titles[$this->id] = ArrayHelper::getValue(Dealer::findOne($this->id), 'id', 'Неизвестно');
        }
